...
impl InputClient {
    pub fn new(ins: u8) -> InputClient {
        let s = alsa::Seq::open(None, Some(Direction::Capture), false)
            .expect("Failed to open sequencer");
        let seqname = CString::new("miditables input").unwrap();
        s.set_client_name(&seqname)
        ...
        for i in 1..ins + 1 {
            ...
            inport.set_type(seq::PortType::MIDI_GENERIC | seq::PortType::APPLICATION);
            inport.set_name(&CString::new(format!("input {}", i)).unwrap());
            s.create_port(&inport).expect("failed to create port");
        }
        InputClient { client: s }
    }
...
impl OutputClient {
    pub fn new(outs: u8) -> OutputClient {
        let s = alsa::Seq::open(None, Some(Direction::Playback), false)
            .expect("Failed to open sequencer");
        let seqname = CString::new("miditables input").unwrap();
        s.set_client_name(&seqname)
            .expect("Failed to set client name");
        for i in 1..outs + 1 {
            ...
            outport.set_type(seq::PortType::MIDI_GENERIC | seq::PortType::APPLICATION);
            outport.set_name(&CString::new(format!("output {}", i)).unwrap());
            s.create_port(&outport).expect("failed to create port");
        }
        OutputClient { client: s }
    }
